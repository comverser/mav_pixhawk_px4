...
async def monitor_ekf(duration: float = 10.0) -> None:
    ...
    while (asyncio.get_event_loop().time() - start_time) < duration:
        # Create async tasks for concurrent telemetry streams
        health_task = asyncio.create_task(_get_health(d))
        position_task = asyncio.create_task(_get_position(d))
        velocity_task = asyncio.create_task(_get_velocity(d))
        attitude_task = asyncio.create_task(_get_attitude(d))

        try:
            health = await health_task
            position = await position_task
            velocity = await velocity_task
            attitude = await attitude_task
